...
def deal_initial_with_trace(deck: Deck, tracer: Tracer, reveal_hole: bool = False) -> Tuple[Hand, Hand, int, Deck]:
    p = Hand.empty()
    d = Hand.empty()

    c, deck = draw_random(deck); p = p.add(c)
    tracer.emit("SYSTEM", "DEAL", c, p.total, d.total, "player card 1")

    c, deck = draw_random(deck); d = d.add(c); upcard = c
    tracer.emit("SYSTEM", "DEAL", c, p.total, d.total, "dealer upcard")

    c, deck = draw_random(deck); p = p.add(c)
    tracer.emit("SYSTEM", "DEAL", c, p.total, d.total, "player card 2")

    c, deck = draw_random(deck); d = d.add(c)
    tracer.emit("SYSTEM", "DEAL", c if reveal_hole else None, p.total, d.total,
...
def play_hand_with_trace(agent, seed: Optional[int] = None, reveal_hole: bool = False) -> Tuple[int, List[Step], Dict[str, Any]]:
    if seed is not None:
        random.seed(seed)
...
def demo_multiple_hands(agent, num_hands: int = 3) -> None:
    ...
    for i in range(1, num_hands + 1):
        payoff, steps, summary = play_hand_with_trace(agent, seed=None, reveal_hole=False)
...
if __name__ == "__main__":
    ...
    BULK_HANDS = 50000
    BULK_SEED: Optional[int] = 1234  # set None for random bulk results
